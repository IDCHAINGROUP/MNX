...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " nodetrade-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  nodetrade-tx [options] <hex-tx> [commands]  Update hex-encoded nodetrade transaction\n" +
                               "or:     nodetrade-tx [options] -create [commands]   Create hex-encoded nodetrade transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded nodetrade transaction
