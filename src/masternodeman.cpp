...
bool CMasternodeMan::CheckInputs(CMasternodeBroadcast& mnb, int nChainHeight, int& nDoS)
{
    ...
    if (collateralUtxo.out.nValue != Params().GetConsensus().GetMNCollateral(nChainHeight)) {
    ...
    // should be at least not earlier than block when 1000 MNX tx got MASTERNODE_MIN_CONFIRMATIONS
...
void ThreadCheckMasternodes()
{
    ...
    util::ThreadRename("nodetrade-masternodeman");
