...
const char * const NODETRADE_CONF_FILENAME = "nodetrade.conf";
const char * const NODETRADE_PID_FILENAME = "nodetrade.pid";
const char * const NODETRADE_MASTERNODE_CONF_FILENAME = "masternode.conf";


// NODETRADE only features
// Masternode
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nodetraded -foo=bar
...
#else
    const char* pszModule = "nodetrade";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NODETRADE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NODETRADE
// Mac: ~/Library/Application Support/NODETRADE
// Unix: ~/.nodetrade
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNX";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NODETRADEParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NODETRADEParams
    // Mac: ~/Library/Application Support/NODETRADEParams
    // Unix: ~/.nodetrade-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MNXParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "MNXParams";
#else
    // Unix
    return pathRet / ".nodetrade-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/nodetrade/sapling-spend.params";
        sapling_output = "/usr/share/nodetrade/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/nodetrade/sapling-spend.params";
            sapling_output = "/usr/local/share/nodetrade/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", NODETRADE_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        // Create empty nodetrade.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(confPath), "a");
        if (configFile != NULL) {
            std::string strHeader = "server=1\n"
                                    "daemon=1\n"
                                    "txindex=1\n"
                                    "rpcport=12690\n"
                                    "port=12689\n";
            ...
            fclose(configFile);
            streamConfig.open(GetConfigFile(confPath));
        }
    }
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", NODETRADE_PID_FILENAME));
