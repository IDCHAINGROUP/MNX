...
CMnxStake* CMnxStake::NewMnxStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        if (nHeight > Params().GetConsensus().height_start_StakeMinAmountEnforcement && coin.out.nValue < Params().GetConsensus().nStakeMinAmount) {
            return nullptr;
        }

        // All good

        return new CMnxStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    if (nHeight > Params().GetConsensus().height_start_StakeMinAmountEnforcement && txPrev->vout[txin.prevout.n].nValue < Params().GetConsensus().nStakeMinAmount) {
        return nullptr;
    }

    // All good

    return new CMnxStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CMnxStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CMnxStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CMnxStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CMnxStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
CDataStream CMnxStake::GetUniqueness() const
{
    //The unique identifier for a MNX stake is the outpoint
...
const CBlockIndex* CMnxStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CMnxStake: uninitialized pindexFrom");
    return pindexFrom;
}
