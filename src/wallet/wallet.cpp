...
/**
 * Fees smaller than this (in umnx) are considered zero fee (for transaction creation)
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    const CAmount collAmt = Params().GetConsensus().GetMNCollateral(chainActive.Height());
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        const CTxOut& txout = tx->vout[i];
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != Params().GetConsensus().GetMNCollateral(chainActive.Height())) {
        strError = strprintf("Invalid collateral tx value, must be %s MNX", FormatMoney(Params().GetConsensus().GetMNCollateral(chainActive.Height())));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-nodetrade-address
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CMnxStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
        ...
        if (pindexPrev->nHeight + 1 > Params().GetConsensus().nTreasuryStartHeight) {
            nCredit += GetBlockValue(pindexPrev->nHeight + 1) - GetDevrewardValue(pindexPrev->nHeight + 1);
        } else {
        ...
        if (pindexPrev->nHeight + 1 > Params().GetConsensus().nTreasuryStartHeight) {
            ...
            txNew.vout.resize(ops + 1);
            txNew.vout[ops].scriptPubKey = payee;
            txNew.vout[ops].nValue = devrewardvalue;
        }
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_NODETRADE && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_NODETRADE);
        }
