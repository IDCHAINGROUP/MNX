...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select MNX Outputs to Spend" :
            "Select Shielded MNX to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::MNX;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show NODETRADE address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
