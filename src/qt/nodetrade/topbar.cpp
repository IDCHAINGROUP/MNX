...
#include "qt/nodetrade/topbar.h"
#include "qt/nodetrade/forms/ui_topbar.h"
#include "qt/nodetrade/lockunlock.h"
#include "qt/nodetrade/qtutils.h"
#include "qt/nodetrade/receivedialog.h"
#include "qt/nodetrade/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/nodetrade/balancebubble.h"
...
TopBar::TopBar(NODETRADEGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    ui->pushButtonConf->setButtonText("nodetrade.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open nodetrade.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("NODETRADE")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralMnx->setText(GUIUtil::formatBalance(CMasternode::GetMNCollateral(chainActive.Tip()->nHeight) * COIN, nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MNX Total
    QString totalMnx = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // MNX
    // Top
    ui->labelAmountTopMnx->setText(totalTransparent);
    ui->labelAmountTopShieldedMnx->setText(totalShielded);
    // Expanded
    ui->labelAmountMnx->setText(totalMnx);
    ui->labelPendingMnx->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureMnx->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
