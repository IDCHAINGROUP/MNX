...
#include "qt/nodetrade/forms/ui_snackbar.h"
#include "qt/nodetrade/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NODETRADEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NODETRADEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
