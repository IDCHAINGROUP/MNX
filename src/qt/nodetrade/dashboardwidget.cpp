...
#include "qt/nodetrade/dashboardwidget.h"
#include "qt/nodetrade/forms/ui_dashboardwidget.h"
#include "qt/nodetrade/sendconfirmdialog.h"
#include "qt/nodetrade/txrow.h"
...
DashboardWidget::DashboardWidget(NODETRADEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareMnx, "square-chart-mnx");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelMnx, "text-chart-mnx");
    ...
    setCssProperty(ui->labelAmountMnx, "text-stake-mnx-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NODETRADEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MNX")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMnx = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MNX, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            mnx = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalMnx += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesMnx.append(mnx);
        chartData->valuesMN.append(mn);

        int max = std::max(mnx, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(16, 27, 199));
    ...
    set0->append(chartData->valuesMnx);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMnx > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountMnx, "text-stake-mnx");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountMnx, "text-stake-mnx-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountMnx, ui->labelAmountMN});
    ui->labelAmountMnx->setText(GUIUtil::formatBalance(chartData->totalMnx, nDisplayUnit));
