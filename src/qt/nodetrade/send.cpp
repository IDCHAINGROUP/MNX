...
#include "qt/nodetrade/send.h"
#include "qt/nodetrade/forms/ui_send.h"
#include "qt/nodetrade/addnewcontactdialog.h"
#include "qt/nodetrade/qtutils.h"
#include "qt/nodetrade/sendchangeaddressdialog.h"
#include "qt/nodetrade/optionbutton.h"
#include "qt/nodetrade/sendconfirmdialog.h"
#include "qt/nodetrade/guitransactionsutils.h"
#include "qt/nodetrade/loadingdialog.h"
...
SendWidget::SendWidget(NODETRADEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a NODETRADE URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-mnx");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this]() { onMNXSelected(true); });
    connect(ui->pushRight, &QPushButton::clicked, [this]() { onMNXSelected(false); });
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MNXs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield MNXs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent MNXs to shield."));
    }
}
...
void SendWidget::onMNXSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto mnxAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(mnxAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(mnxAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(mnxAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
