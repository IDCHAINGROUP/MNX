...
#include "qt/nodetrade/forms/ui_addresseswidget.h"
#include "qt/nodetrade/addressfilterproxymodel.h"
#include "qt/nodetrade/addresslabelrow.h"
#include "qt/nodetrade/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(NODETRADEGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto mnxAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(mnxAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(mnxAdd);
        if (walletModel->updateAddressBookLabels(mnxAdd, label.toUtf8().constData(),
