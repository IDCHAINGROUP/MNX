...
#include "qt/nodetrade/splash.h"
#include "qt/nodetrade/welcomecontentwidget.h"
...
#ifdef ENABLE_WALLET
#include "qt/nodetrade/governancemodel.h"
#include "qt/nodetrade/mnmodel.h"
...
{
    return QCoreApplication::translate("nodetrade-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nodetrade.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nodetrade.qrc)
...
/** Class encapsulating NODETRADE startup and shutdown.
...
/** Main NODETRADE application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NODETRADEGUI)
...
{
    window = new NODETRADEGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NODETRADEGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NODETRADEGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(NODETRADEGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NODETRADEGUI::DEFAULT_WALLET);
        ...
        // NODETRADE: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NODETRADEGUI::handlePaymentRequest);
        connect(window, &NODETRADEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. NODETRADE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nodetrade_locale);
    Q_INIT_RESOURCE(nodetrade);
    ...
    /// 6. Determine availability of data and blocks directory and parse nodetrade.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", NODETRADE_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // nodetrade: links repeatedly have their payment requests routed to this process:
