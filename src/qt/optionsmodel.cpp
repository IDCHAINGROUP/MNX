...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MNX);
    ...
    if (settings.contains("nAnonymizeNodetradeAmount") || reset)
        gArgs.SoftSetArg("-anonymizenodetradeamount", settings.value("nAnonymizeNodetradeAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nodetrade.cpp during shotdown to also remove the window positions
