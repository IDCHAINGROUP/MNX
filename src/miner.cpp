...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("NODETRADEMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("NODETRADEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("NODETRADEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("nodetrade-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running NODETRADEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("NODETRADEMiner exception");
    } catch (...) {
        LogPrintf("NODETRADEMiner exception");
    }

    LogPrintf("NODETRADEMiner exiting\n");
}
