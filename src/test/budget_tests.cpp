// Copyright (c) 2018-2020 The NODETRADE developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "test_nodetrade.h"
...
BOOST_AUTO_TEST_CASE(budget_value)
{
    ...
    int nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_V3_4].nActivationHeight + 1;
    CheckBudgetValue(nHeightTest-1, "testnet", 7200*COIN);
    CheckBudgetValue(nHeightTest, "testnet", 144*COIN);

    SelectParams(CBaseChainParams::MAIN);
    nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_V3_4].nActivationHeight + 1;
...
{
    const CAmount mnAmt = GetMasternodePayment(nHeight, GetBlockValue(nHeight));
