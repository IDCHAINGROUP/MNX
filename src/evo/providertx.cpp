...
static bool CheckCollateralOut(const CTxOut& out, const ProRegPL& pl, CValidationState& state, CTxDestination& collateralDestRet, int nHeight)
{
    ...
    if (out.nValue != Params().GetConsensus().GetMNCollateral(nHeight)) {
...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pl.collateralOutpoint.hash.IsNull()) {
        ...
        if (!CheckCollateralOut(tx.vout[pl.collateralOutpoint.n], pl, state, collateralTxDest, pindexPrev->nHeight)) {
            // pass the state returned by the function above
            return false;
        }
        ...
        CTxDestination collateralTxDest;
        if (!CheckCollateralOut(coin.out, pl, state, collateralTxDest, pindexPrev->nHeight)) {
            // pass the state returned by the function above
            return false;
        }
