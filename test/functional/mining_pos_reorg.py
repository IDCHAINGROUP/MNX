...
class ReorgStakeTest(NodetradeTestFramework):
    ...
    def check_money_supply(self, expected_mnx):
        # verify that nodes have the expected MNX supply
        mnx_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(mnx_supply, [DecimalAmt(expected_mnx)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that MNX supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check MNX supply...")
