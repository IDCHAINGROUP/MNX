...
- [Development guidelines](#development-guidelines)
    - [General NODETRADE](#general-nodetrade-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NODETRADE uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MNXs (tMNX)" on the test network, if you
...
NODETRADE is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/nodetrade-project/nodetrade/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_nodetrade
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_nodetrade --log_level=test_suite
$ valgrind -v --leak-check=full src/nodetraded -printtoconsole
```
...
# A coverage report will now be accessible at `./test_nodetrade.coverage/index.html`.
```

**Sanitizers**
NODETRADE can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates MNXs (if wallet is enabled).
...
However, in open source software such as NODETRADE, where everyone uses
...
pay attention to for reviewers of NODETRADE code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NODETRADE uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against NODETRADE so that they can be integrated
...
file descriptors it will cause problems with NODETRADE's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:NODETRADE-Project/NODETRADE.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `nodetrade-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `nodetrade-cli` and the GUI debug console use this table to determine how to
