...
*Setup instructions for a gitian build of NODETRADE using a VM or physical system.*

Gitian is the deterministic build process that is used to build the NODETRADE
...
to the NODETRADE GitHub release page.
...
- [Building NODETRADE](#building-nodetrade-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/nodetrade-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/nodetrade-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/nodetrade-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/nodetrade-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/nodetrade-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/nodetrade-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/nodetrade-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@nodetrade.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/nodetrade-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/nodetrade-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/nodetrade-osx-*-build.assert
```
