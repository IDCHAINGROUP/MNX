...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/NODETRADEParams"
    else
        PARAMS_DIR="$HOME/.nodetrade-params"
    fi
fi
...
Failed to fetch the NODETRADE zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
NODETRADE - fetch-params.sh

This script will fetch the NODETRADE zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common NODETRADE zkSNARK parameters. Note that it is
